...
# ══════════════════════════════════════════════════════════════════════════════
# Paper Section IV-C: attack family mapping for severity & family classification
ATTACK_FAMILIES = {
    "normal":   "Normal",
    ...
    "ftp_write":"R2L",   "imap":    "R2L",    "multihop": "R2L",
    "phf":      "R2L",   "spy":     "R2L",
    "rootkit":  "U2R",   "buffer_overflow":"U2R", "loadmodule":"U2R",
    "perl":     "U2R",   "sqlattack":"U2R",   "xterm":    "U2R",
    "ps":       "U2R",
}

# Paper Section IV-D & Table VII: RAG Knowledge Base — 6 primary attack entries
# Snort rules match paper Table VII exactly; severity per paper Table VII
KB = {
    "neptune": {
        "title": "Neptune / SYN Flood (DoS)",
        "body": (
            "Neptune attacks send thousands of SYN packets without completing the "
            "TCP handshake, exhausting server connection tables. "
    ...
    "smurf": {
        ...
        "body": (
            "ICMP echo requests sent to broadcast address with spoofed victim IP. "
            "All hosts reply to victim, amplifying bandwidth. "
            "Mitigation: disable IP-directed broadcasts, BCP38 ingress filtering."
        ),
        "tags": ["DDoS", "ICMP", "Amplification", "Spoofing"],
        "snort_rule": (
            'alert icmp any any -> $HOME_NET any '
            '(itype:8; msg:"ICMP Smurf Attack";)'
        ),
        "severity": "high",
    },
    "portsweep": {
        "title": "Port Sweep / Reconnaissance",
        "body": (
            "Attacker scans multiple ports to identify open services. "
            "High connection count with diverse destination ports. "
            "Mitigation: port knocking, firewall, IDS rules."
        ),
    ...
    "ipsweep": {
        ...
        "body": (
            "Scans multiple IPs via ICMP ping to map live hosts. "
            "High ICMP request count from single source. "
            "Mitigation: ICMP rate limiting, honeypots."
        ),
        "tags": ["Probe", "Reconnaissance", "ICMP"],
        "snort_rule": (
            'alert icmp any any -> $HOME_NET any '
            '(itype:8; threshold:type both,track by_src,count 15,seconds 3; '
            'msg:"IP Sweep";)'
        ),
        "severity": "medium",
    },
    "buffer_overflow": {
        ...
        "body": (
            "Exploits memory management flaws to inject and execute code with "
            "elevated privileges. Indicators: root_shell=1, num_root>0. "
            "Mitigation: ASLR, stack canaries, NX bit."
        ),
        "tags": ["U2R", "Memory-Exploit", "Privilege-Escalation"],
        "snort_rule": (
            'alert tcp any any -> $HOME_NET any '
            '(content:"|90 90 90|"; msg:"Possible Buffer Overflow";)'
        ),
        "severity": "high",
    },
    "guess_passwd": {
        ...
        "body": (
            "Repeated login attempts with credential lists. "
            "High num_failed_logins, logged_in=0. "
            "Mitigation: account lockout, MFA, fail2ban, CAPTCHA."
        ),
    ...
    "normal": {
        ...
        "body": (
            "Traffic conforms to expected baseline. No anomaly detected. "
...
def _severity(label: str) -> str:
    """
    Paper Table VII severity mapping:
      U2R  → HIGH
      DoS  → HIGH  (neptune, smurf)
      Probe → MEDIUM (portsweep, ipsweep)
      R2L  → MEDIUM (guess_passwd)
      Normal → INFO
    """
    label_clean = label.strip().lower().rstrip(".")
    # Direct KB lookup first (most precise)
    for k in KB:
        if k in label_clean:
            return KB[k]["severity"]
    f = _family(label_clean)
...
# ══════════════════════════════════════════════════════════════════════════════
# DATASET  (paper Section V-A: 8,000-record KDDTrain+ subsample)
# ══════════════════════════════════════════════════════════════════════════════
NSL_COLS = [
...
def load_dataset():
    """
    Paper Section V-A: Load 8,000-record KDDTrain+ subsample.
    Fallback: synthetic 5,000-record dataset with numpy distributions
    calibrated to empirical NSL-KDD feature statistics.
    """
    for src, url in [
        ("NSL-KDD (KDDTrain+)",
    ...
    # Offline synthetic fallback (paper Section V-A)
    np.random.seed(42)
    n = 5000
    lp = [
    ...
    df = pd.DataFrame({
        ...
        "service":        np.random.choice(
            ["http", "ftp", "smtp", "ssh", "dns", "telnet", "private", "other"], n,
            p=[0.3, 0.1, 0.1, 0.14, 0.1, 0.05, 0.11, 0.10],
        ),
        "flag":           np.random.choice(
            ["SF", "S0", "REJ", "RSTO", "SH", "OTH"], n,
...
# Paper Section VI-A/B: RF & IF hyperparameters; Section V-C: preprocessing
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_resource(show_spinner=True)
def _boot_train():
    df, src = load_dataset()

    # Step 2 — Categorical encoding (paper Section V-C Step 2)
    ...
    # Step 3 — Binary target construction (paper Section V-C Step 3)
    d["is_attack"] = (d["label"].str.strip().str.lower() != "normal").astype(int)

    # Feature list excludes label, difficulty_level, is_attack
    feat = [c for c in d.columns if c not in ["label", "difficulty_level", "is_attack"]]

    # Step 4 — Missing value imputation: zero-fill (paper Section V-C Step 4)
    X = d[feat].fillna(0)
    y = d["is_attack"]

    # Step 5 — StandardScaler normalization (paper Section V-C Step 5)
    sc = StandardScaler()
    Xs = sc.fit_transform(X)

    # Step 6 — Stratified 70/30 split (paper Section V-C Step 6)
    # test_size=0.30, random_state=13, stratify=y
    Xtr, Xte, ytr, yte = train_test_split(
        Xs, y, test_size=0.30, random_state=13, stratify=y
    )

    # Paper Section VI-A — Random Forest hyperparameters (Table III)
    # n_estimators=45, max_depth=10, min_samples_leaf=6,
    # max_features="sqrt" (≈√41≈6), random_state=99, n_jobs=-1
    ...
    # Paper Section VI-B — Isolation Forest hyperparameters (Table III)
    # n_estimators=80, contamination=0.22
    # Anomaly threshold θ = −0.05 applied at inference time
    iso = IsolationForest(n_estimators=80, contamination=0.22, random_state=42)
    iso.fit(Xtr)

    # Evaluation metrics on 30% held-out test split (paper Section VII-C)
    yp = rf.predict(Xte)
    ...
    return rf, iso, sc, les, feat, acc, f1, prec, rec, cm, fp_rate, fn_rate, df, src
...
# ══════════════════════════════════════════════════════════════════════════════
# SNORT SIMULATION
# Paper Section VI-C — exact rule thresholds
# ══════════════════════════════════════════════════════════════════════════════
_SNORT = {
    # Neptune/SYN Flood: serror_rate > 0.70 AND count > 100
    "neptune":       lambda r: float(r.get("serror_rate", 0)) > 0.70 and int(r.get("count", 0)) > 100,
    # Smurf/ICMP DDoS: protocol_type == "icmp" AND src_bytes > 9,000
    "smurf":         lambda r: str(r.get("protocol_type", "")) == "icmp" and int(r.get("src_bytes", 0)) > 9000,
    # Portsweep: diff_srv_rate > 0.50 AND count > 30
    "portsweep":     lambda r: float(r.get("diff_srv_rate", 0)) > 0.50 and int(r.get("count", 0)) > 30,
    # IPSweep: protocol_type == "icmp" AND dst_host_count > 20
    "ipsweep":       lambda r: str(r.get("protocol_type", "")) == "icmp" and int(r.get("dst_host_count", 0)) > 20,
    # Buffer Overflow: root_shell == 1 OR num_root > 1
    "buffer_overflow": lambda r: int(r.get("root_shell", 0)) == 1 or int(r.get("num_root", 0)) > 1,
    # Guess_passwd: num_failed_logins ≥ 3
    "guess_passwd":  lambda r: int(r.get("num_failed_logins", 0)) >= 3,
}


def _snort(row: dict) -> str:
    """Returns attack label if any Snort rule fires, else 'normal'."""
...
# Anomaly threshold θ = −0.05  (paper Section IV-C, calibrated via ROC / F2-score)
...
def _sim_pkt(row: dict) -> dict:
    return dict(
        ts=datetime.now().strftime("%H:%M:%S.%f")[:-3],
        src=_fake_ip(),
        dst="192.168.1." + str(np.random.randint(1, 50)),
...
def _tstr(row: dict) -> str:
    """
    Paper Section IV-B: _tstr() converts 41-dimensional feature vector into a
    structured natural-language paragraph summarizing protocol type, service,
    connection flags, byte counts, error rates, login behavior, and shell
    activity indicators.
    """
    return (
...
def _vec(row: dict) -> np.ndarray:
    """Transform a raw feature dict into a StandardScaler-normalized numpy array."""
    tmp = {}
    for c in FEAT:
        val = row.get(c, 0)
        if c in LES:
...
def _is_alert(rf_pred: int, iso_score: float) -> bool:
    """
    Paper Section IV-C hybrid alert logic:
      alert(x) = 1  if  (ŷ_RF(x) = 1)  OR  (d(x) < θ = −0.05)
               = 0  otherwise
    """
    return bool(rf_pred == 1) or bool(iso_score < ANOMALY_THRESHOLD)


# ══════════════════════════════════════════════════════════════════════════════
# LLM FUNCTIONS
# ══════════════════════════════════════════════════════════════════════════════
def llm_report(client, summary: str, label: str, kb: dict) -> str:
    """
    Paper Section IV-B & VI-D:
      Model  : llama3-70b-8192  (Groq)
      Temperature : 0.25  (minimises hallucination, maximises factual precision)
      max_tokens  : 500   (sufficient for complete 5-section report)
      Output: exactly 5 labelled sections, ≤280 words
    """
    prompt = (
        "You are an expert cybersecurity analyst reviewing network traffic "
        "flagged by an ML-based IDS.\n\n"
        f"=== TRAFFIC SUMMARY ===\n{summary}\n\n"
        "=== ML CLASSIFICATION ===\n"
        f"Attack Type   : {label}\n"
        f"Attack Family : {_family(label)}\n"
        f"Severity      : {_severity(label).upper()}\n"
        f"Known Pattern : {kb['title']}\n\n"
        f"=== SNORT SIGNATURE ===\n{kb['snort_rule']}\n\n"
        ...
        "**4. LLM vs Signature IDS** — 1 sentence comparing Snort vs LLM analysis.\n"
...
def llm_soc(client, reports: list) -> str:
    """
    Paper Section IV-C & VI-D:
      Model       : llama3-8b-8192  (Groq)
      Temperature : 0.30  (natural-sounding prose for SOC briefings)
      Context     : last 5 alerts  (paper equation: SOC_Brief = LLaMA3_8B({alert_i}_{i=N-4}^{N}))
      Output      : 3-4 sentence prose briefing covering threat posture,
                    dominant attack types, and recommended immediate actions
    """
    # Paper: SOC briefing covers the last 5 alerts
    last = reports[-5:]
    lines = "\n".join(
        f"- {r['ts']}: {r['label']} ({r['sev'].upper()}) on {r['svc']}"
        for r in last
    )
    r = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": (
                "You are a SOC analyst. Write a 3-4 sentence shift briefing covering "
                "threat posture, dominant attack types, and urgent actions. Prose only.\n\n"
                f"Recent alerts:\n{lines}"
            ),
        }],
        max_tokens=200,
...
# SIDEBAR NAV
# ══════════════════════════════════════════════════════════════════════════════
# Paper Section VII-B: 6 analytical tabs
NAV_PAGES = [
    "📊  Dataset & Model",
    "📡  Live Capture",
    "🤖  LLM Threat Reports",
...
with st.sidebar:
    ...
    key_ok = _llm_ready()
    key_txt = (
        '<span style="color:#4ade80;">🟢 API key ready</span>'
        if key_ok
        else '<span style="color:#f87171;">🔴 Add GROQ_API_KEY</span>'
    )
    st.markdown(f"""
...
<div class="hero">
  ...
  <div style="flex:1;">
    ...
    <div class="hero-sub">
      NSL-KDD · Groq LLaMA-3 70B · Random Forest · Isolation Forest · RAG · NLP
    </div>
  </div>
...
if page == "📊  Dataset & Model":
    ...
    with c1:
        ...
        fig = px.bar(fc, x="Family", y="Count", color="Family",
    ...
    # Confusion matrix — paper Fig. 4 labels
    ...
    # Feature importance — paper Section VIII-D: top-15, mean Gini decrease
    st.markdown('<p class="sec">Feature Importance — Top 15 (Mean Gini Decrease)</p>',
                unsafe_allow_html=True)
    ...
    # Model metrics KPI row — paper Table III/IV
    st.markdown(f"""
    <div class="kpi-row">
      <div class="kpi blue">
      ...
      <div class="kpi amber">
        <div class="kpi-label">FP Rate</div>
        <div class="kpi-val">{FP_RATE*100:.1f}%</div>
        <div class="kpi-sub">RF classifier</div>
      </div>
      <div class="kpi red">
        <div class="kpi-label">FN Rate</div>
        <div class="kpi-val">{FN_RATE*100:.1f}%</div>
        <div class="kpi-sub">RF classifier</div>
      </div>
    </div>
    <div class="kpi-row">
      <div class="kpi amber">
        <div class="kpi-label">RF Trees</div>
        <div class="kpi-val">45</div>
        <div class="kpi-sub">max_depth=10</div>
      </div>
      <div class="kpi red">
        <div class="kpi-label">IF Trees</div>
        <div class="kpi-val">80</div>
        <div class="kpi-sub">contamination=0.22</div>
      </div>
      <div class="kpi blue">
        <div class="kpi-label">IF Threshold θ</div>
        <div class="kpi-val">−0.05</div>
        <div class="kpi-sub">ROC / F₂-score</div>
      </div>
      <div class="kpi green">
        <div class="kpi-label">Test Split</div>
        <div class="kpi-val">70/30</div>
        <div class="kpi-sub">Stratified</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("🔍 Raw Dataset Sample (100 rows)"):
        st.dataframe(
            DF.sample(min(100, len(DF)), random_state=1).reset_index(drop=True),
...
# ── PAGE 2: Live Capture ──────────────────────────────────────────────────────
    ...
    st.info(
        "Simulates live packet capture. In production replace with "
        "`scapy.sniff()` or `pyshark.LiveCapture()`.",
        icon="ℹ️",
    )
    ...
    if st.button("▶️  Start Capture", use_container_width=True):
        ...
        for row in rows:
            ...
            iso_score = float(ISO.decision_function(xv)[0])

            # Paper Section IV-C: hybrid alert = RF=1 OR d(x) < θ
            alert_flag = _is_alert(rf_pred, iso_score)
            anomaly_flag = iso_score < ANOMALY_THRESHOLD

            pkt.update(
                pred_attack=rf_pred,
                anomaly=anomaly_flag,
                alert=alert_flag,           # combined OR alert per paper
            ...
            html = "".join(
                ...
                f'[{p["ts"]}] <span>{p["src"]}</span>→<span>{p["dst"]}</span>'
                f' | {p["proto"]}/{p["svc"]}/{p["flag"]} | {p["length"]:,}B'
                f' | <span style="color:{"#f87171" if p["alert"] else "#4ade80"}">'
                f'{"ALERT" if p["alert"] else "NORMAL"}</span>'
                f'{"⚠️" if p["anomaly"] and not p["pred_attack"] else ""}'
                f'</div>'
        ...
        alerts = sum(p["alert"] for p in captured)
        rf_atks = sum(p["pred_attack"] for p in captured)
        anoms = sum(p["anomaly"] for p in captured)
        st.success(
            f"✅ {n_pkts} packets — {alerts} alert(s) "
            f"(RF: {rf_atks}, IF: {anoms}, hybrid OR: {alerts})"
        )

    if st.session_state._capture:
        cap = st.session_state._capture
        alerts = sum(p["alert"] for p in cap)
        rf_atks = sum(p["pred_attack"] for p in cap)
        anoms = sum(p["anomaly"] for p in cap)

        st.markdown(f"""<div class="kpi-row">
          <div class="kpi blue">
            <div class="kpi-label">Packets</div>
            <div class="kpi-val">{len(cap)}</div>
          </div>
          <div class="kpi red">
            <div class="kpi-label">Hybrid Alerts</div>
            <div class="kpi-val">{alerts}</div>
            <div class="kpi-sub">RF OR IF</div>
          </div>
          <div class="kpi amber">
            <div class="kpi-label">RF Attacks</div>
            <div class="kpi-val">{rf_atks}</div>
            <div class="kpi-sub">Supervised</div>
          </div>
          <div class="kpi green">
            <div class="kpi-label">IF Anomalies</div>
            <div class="kpi-val">{anoms}</div>
            <div class="kpi-sub">Unsupervised θ={ANOMALY_THRESHOLD}</div>
          </div>
        </div>""", unsafe_allow_html=True)

        # IF score timeline — paper Fig. 5
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                color=["#ef4444" if p["alert"] else "#22c55e" for p in cap],
                size=8,
            ),
        ))
        fig.add_hline(
            ...
            line_dash="dash",
            line_color="#f59e0b",
            annotation_text=f"Anomaly threshold θ = {ANOMALY_THRESHOLD}",
        )
        fig.update_layout(
            **PLOT,
            title="Isolation Forest Decision Function Score per Packet",
            xaxis_title="Packet #",
            yaxis_title="d(x) = IF.decision_function(x)",
        )
        st.plotly_chart(fig, use_container_width=True)
...
# ── PAGE 3: LLM Threat Reports ────────────────────────────────────────────────
    ...
    st.markdown(
        '<p class="sec">🤖 LLM Threat Reports — Groq LLaMA-3 70B</p>',
        unsafe_allow_html=True,
    )
    if not _llm_ready():
        ...
        if st.button("🔍  Analyze & Generate Reports", use_container_width=True):
            ...
            samples = pool.sample(min(int(n_analyze), len(pool)),
            ...
            for idx, (_, row) in enumerate(samples.iterrows()):
                ...
                # Paper Section IV-C: trigger report on hybrid alert (RF OR IF)
                alert_flag = _is_alert(rf_pred, iso_score)
                pred_label = true_lbl if rf_pred else "normal"
                ...
                st.session_state.reports.insert(0, dict(
                    ...
                    sev=sev,
                    svc=rd.get("service", "?"),
                    proto=str(rd.get("protocol_type", "?")),
                    report=rtext,
                    kb=kb,
                    alert=alert_flag,
                    iso_score=round(iso_score, 3),
        ...
        for r in st.session_state.reports[:20]:
            ...
            <div class="alert-card {sc_cls}">
              <div class="alert-meta">
                🕐 {r['ts']} | {r['proto'].upper()}/{r['svc']} |
                True: <b>{r['true_label']}</b> |
                IF score: <b>{r.get('iso_score', 'N/A')}</b>
              </div>
              <div class="alert-title">
                {r['kb']['title']}
                <span class="pill pill-{r['sev']}">{r['sev'].upper()}</span>
              </div>
...
# ── PAGE 4: IDS Comparison ────────────────────────────────────────────────────
    ...
    st.markdown(
        '<p class="sec">📈 ML-IDS vs Traditional Signature IDS — Snort / Suricata</p>',
        unsafe_allow_html=True,
    )
    ...
    if st.button("▶️  Run Comparison", use_container_width=True):
        ...
        for i, row in enumerate(rows):
            ...
            rf_pred = int(RF.predict(xv)[0])
            iso_score = float(ISO.decision_function(xv)[0])
            # Hybrid alert for ML-IDS column
            ml_p.append(1 if _is_alert(rf_pred, iso_score) else 0)
        ...
        sn_fn = sum(1 for m, t in zip(sn_p, true_b) if m == 0 and t == 1)
        tn_c  = sum(1 for m, t in zip(ml_p, true_b) if m == 0 and t == 0)
        tp_c  = sum(1 for m, t in zip(ml_p, true_b) if m == 1 and t == 1)

        st.session_state._cmp = dict(
            ml_acc=accuracy_score(true_b, ml_p),
            sn_acc=accuracy_score(true_b, sn_p),
            ...
            ml_fp=ml_fp, sn_fp=sn_fp,
            ml_fn=ml_fn, sn_fn=sn_fn,
            ml_fp_rate=ml_fp / (ml_fp + tn_c) if (ml_fp + tn_c) > 0 else 0,
            sn_fp_rate=sn_fp / (sn_fp + sum(1 for m, t in zip(sn_p, true_b) if m == 0 and t == 0))
                if sum(1 for m, t in zip(sn_p, true_b) if m == 0 and t == 0) + sn_fp > 0 else 0,
            n=n_s,
        )
    ...
    if st.session_state._cmp:
        ...
        st.markdown(f"""<div class="kpi-row">
          <div class="kpi blue">
            ...
            <div class="kpi-val">{c['ml_acc']*100:.1f}%</div>
            <div class="kpi-sub">RF + IF hybrid</div>
          </div>
          <div class="kpi amber">
            <div class="kpi-label">Snort Accuracy</div>
            <div class="kpi-val">{c['sn_acc']*100:.1f}%</div>
            <div class="kpi-sub">Signature IDS</div>
          </div>
          <div class="kpi green">
            <div class="kpi-label">ML F1</div>
            <div class="kpi-val">{c['ml_f1']:.3f}</div>
            <div class="kpi-sub">Weighted</div>
          </div>
          <div class="kpi red">
            <div class="kpi-label">Snort F1</div>
            <div class="kpi-val">{c['sn_f1']:.3f}</div>
            <div class="kpi-sub">Weighted</div>
          </div>
        </div>""", unsafe_allow_html=True)

        # Performance comparison bar chart — paper Fig. 7
        ...
        fig.add_trace(go.Bar(
            name="RF+IF Hybrid (ML-IDS)", x=metrics, y=ml_vals,
            marker_color="#3b82f6",
            text=[f"{v:.1f}%" for v in ml_vals], textposition="outside",
        ))
        fig.add_trace(go.Bar(
            name="Snort Signatures", x=metrics, y=sn_vals,
            marker_color="#f59e0b",
            text=[f"{v:.1f}%" for v in sn_vals], textposition="outside",
        ))
        fig.update_layout(
            **PLOT, barmode="group", yaxis_range=[0, 118],
            title=f"ML-IDS vs Signature IDS — {c['n']} samples",
        )
        fig.update_yaxes(showgrid=True, gridcolor="#1e2d45")
        st.plotly_chart(fig, use_container_width=True)

        # FP / FN analysis — paper Fig. 8 (error analysis)
        fig2 = go.Figure()
        ...
        with r1:
            st.markdown(f"""<div class="rag-card">
              <div class="rag-title">🤖 RF + IF Hybrid (ML-IDS)</div>
              <div class="rag-body">
                Acc: <b>{c['ml_acc']*100:.1f}%</b> | F1: <b>{c['ml_f1']*100:.1f}%</b><br>
                FP: <b>{c['ml_fp']}</b> | FN: <b>{c['ml_fn']}</b> |
                FP Rate: <b>{c['ml_fp_rate']*100:.1f}%</b><br><br>
                Learns statistical patterns from 41 NSL-KDD features.
                Hybrid OR-logic (RF + IF) detects novel zero-day attacks.
              </div>
            </div>""", unsafe_allow_html=True)
        with r2:
            st.markdown(f"""<div class="rag-card">
              ...
              <div class="rag-body">
                Acc: <b>{c['sn_acc']*100:.1f}%</b> | F1: <b>{c['sn_f1']*100:.1f}%</b><br>
                FP: <b>{c['sn_fp']}</b> | FN: <b>{c['sn_fn']}</b> |
                FP Rate: <b>{c['sn_fp_rate']*100:.1f}%</b><br><br>
                Rule-based, fast, reliable on known CVEs.
                Blind to zero-day and polymorphic attacks.
              </div>
            </div>""", unsafe_allow_html=True)

        # Capability radar — paper Fig. 8 (six-axis radar)
        sc_factor = max(c["n"] / 100, 1)
        cats = ["Accuracy", "F1 Score", "Low FP", "Low FN",
                "Novel Attacks", "Speed"]
        ml_r = [
            c["ml_acc"] * 100, c["ml_f1"] * 100,
            max(0, 100 - c["ml_fp"] / sc_factor * 8),
            max(0, 100 - c["ml_fn"] / sc_factor * 8),
            83, 52,  # paper Table VI: zero-day ✓, latency 28ms
        ]
        sn_r = [
            c["sn_acc"] * 100, c["sn_f1"] * 100,
            max(0, 100 - c["sn_fp"] / sc_factor * 8),
            max(0, 100 - c["sn_fn"] / sc_factor * 8),
            17, 96,  # paper Table VI: zero-day ✕, latency 4ms → speed 96
        ]
        fig3 = go.Figure()
        fig3.add_trace(go.Scatterpolar(
            r=ml_r, theta=cats, fill="toself",
            name="ML-IDS", line_color="#3b82f6",
        ))
        fig3.add_trace(go.Scatterpolar(
            r=sn_r, theta=cats, fill="toself",
        ...
        fig3.update_layout(
            **PLOT,
            ...
            title="Six-Axis Capability Radar",
        )
        st.plotly_chart(fig3, use_container_width=True)
...
# ── PAGE 5: RAG Knowledge Base ────────────────────────────────────────────────
    ...
    st.markdown(
        '<p class="sec">🧠 RAG-Based Threat Knowledge Base</p>',
        unsafe_allow_html=True,
    )
    st.info(
        "KB entries are injected as context when the LLM generates reports, "
        "grounding responses in known attack signatures (paper Section IV-D).",
        icon="ℹ️",
    )
    ...
    if q.strip():
        ...
        entries = [
            ...
            if ql in k or ql in v["title"].lower() or ql in v["body"].lower()
        ]

    for key, entry in entries:
        fam = _family(key)
        sev = entry["severity"]      # Use KB-stored severity (paper Table VII)
        tags = "".join(
            f'<span class="rag-tag">{t}</span>' for t in entry["tags"]
        )
        st.markdown(f"""<div class="rag-card">
          <div class="rag-title">
            {entry['title']}
          ...
          <div style="margin-top:10px;">
            <span style="font-family:'JetBrains Mono',monospace;font-size:.65rem;color:#64748b;">
              SNORT RULE:
            </span><br>
            <code style="font-size:.7rem;color:#a5b4fc;background:#0d1220;
                         padding:6px 10px;border-radius:6px;display:block;
                         margin-top:4px;border:1px solid #1e2d45;">
              {entry['snort_rule']}
            </code>
          </div>
    ...
    n_sev = st.selectbox("Severity", ["high", "medium", "low", "info"], index=0)
...
# ── PAGE 6: NLP SOC Summary ───────────────────────────────────────────────────
    ...
    st.markdown(
        '<p class="sec">📋 NLP SOC Briefing — Groq LLaMA-3 8B</p>',
        unsafe_allow_html=True,
    )
    if not _llm_ready():
        ...
        st.warning(
            "⚠️  Generate at least one report on the **LLM Threat Reports** page first."
        )
    else:
        rpts = st.session_state.reports
        st.markdown(
            f"**{len(rpts)} alert(s) in queue.** "
            "Briefing covers last 5 alerts (paper Section IV-C)."
        )
        ...
        if st.session_state._brief:
            st.markdown(f"""<div class="alert-card info">
              <div class="alert-meta">
                ...
                Groq LLaMA-3 8B | SOC Briefing (last 5 alerts)
              </div>
              <div class="alert-title">
                SOC Threat Summary
                <span class="pill pill-info">NLP</span>
              </div>
        ...
        for r in rpts[:15]:
            ...
            st.markdown(f"""<div class="alert-card {sc_cls}" style="padding:12px 18px;">
              ...
              <div class="alert-title" style="font-size:.86rem;">
                {r['kb']['title']}
                <span class="pill pill-{r['sev']}">{r['sev'].upper()}</span>
              </div>
            </div>""", unsafe_allow_html=True)
